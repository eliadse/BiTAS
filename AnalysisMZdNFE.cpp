...
int AnalysisMZdNFE(const char *filename, const double dispCut, const int binwcoarse, const char *period){
...
//    }
    ...
    maps->Close();
    std::cout << "Epsilon_ref MidZd = " << epsilonref << std::endl;
...
//    for (int j = 0; j < 3; j++){
        ...
        for (int m = 1; m <= hcell->GetNbinsX(); m++){
            ...
            Results->Fill(val, expv, 35.01, mjdbin, tnum, vE/epsilonref);       
            lastvE = vE; 
        }
