...
int RingCollectionArea_MZd(const char *period){
    // To test if I can get the same results as in the upgrade paper I would have to optimize the hadronness cut for the Crab
    ...
    char *direction = Form("/data/magic/users-ifae/edosouto/MAGIC/BigSearch/Ringwobble_MC/%s/za35to50", period);

    std::cout << "File: " << Form("%s/GA_*.root", direction) << std::endl;
    ...
    TH1D* hAeff1 = new TH1D("hAeff1","hAeff1", nFineTrue, log10(minE),log10(maxE));
    ...
    std::cout << "Maximum impact radius: " << maxr << " " << maxr << std::endl;
    ...
    for (int ii = 0; ii < data1->GetEntries(); ii++) {
        ...
        if (eest > 10000 || eest < 200) continue;
    ...
    outputFile2.open(Form("/data/magic/users-ifae/edosouto/MAGIC/BigSearch/Melibea/%s/35to50/AEff_%.0fto%.0fGeVin%ibinsza35to50EestCut.txt", period, minE, maxE, nFineTrue));
    outputFile2 << "# TrueEnergy"; 

    outputFile2 << " Aeff[35deg to 50deg Zd]";
    outputFile2 << "\n";
...
//            hAeff1[ii]->SetBinContent(mm+1, Aeff[ii][mm]);
...
//    }    
    ...
    for (int mm = 0; mm<nFineTrue; mm++){
        ...
        outputFile2 << edgesEnt[mm+1] << " "; 
            ...
            hAeff1->SetBinContent(mm+1, Aeff[mm]);
...
/*
  ...
  */  
    ...
    hAeff1->SetTitle("35deg to 50deg");
    hAeff1->GetXaxis()->SetTitle("log(E/GeV)");
    hAeff1->GetYaxis()->SetTitle("\\mbox{Collection area } [m^{2}]"); 

    hAeff1->SetLineColor(kPink);
    hAeff1->Draw();
    cmaster->SaveAs(Form("/data/magic/users-ifae/edosouto/MAGIC/BigSearch/Melibea/%s/35to50/AEff_%.0fto%.0fGeVin%ibinsza35to50EestCut.png", period, minE, maxE, nFineTrue));
//    cmaster->SaveAs(Form("AEff_%.0fto%.0fGeVin%ibinsza35to50EesCut.png",minE, maxE, nFineTrue)); 
