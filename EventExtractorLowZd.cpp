...
int EventExtractorLowZd(const char *filename, const double dispCut, const char *period){
    ...
    for (int i = 0; i < numEntries; i++){
        ...
        if (EndEvent < 70. || EndEvent > 10000.) continue;
    ...
    // Create a file only if there is something to save (data has contents)
if(filled){
    TFile *f0l = new TFile(Form("/data/magic/users-ifae/edosouto/MAGIC/BigSearch/Code/MARS3version/%s/05to35/EL_%s_%s_%s_%.1f.root", period, night, source, run, dispCut),"recreate");
    std::cout << "f0 low name:  " << f0l->GetName() << "\n";
